...
async function init() {
  ...
  const draft = (await extensionApi.storage.local.get(DRAFT_KEY))[DRAFT_KEY] ?? {};
  ...
  for (const id of ["backend-url", "import-key", "player-id"]) {
    field(id).addEventListener("input", () => {
      extensionApi.storage.local.set({ [DRAFT_KEY]: connectionFields() }).catch(error => notice(errorText(error), "error"));
    });
  }
...
function permissionOrigin(origin: string) { const url = new URL(origin); return url.protocol + "//" + url.hostname + "/*"; }
async function updateConnection() {
  connectionReady = false;
  try {
    connectionReady = !!settings.faceitPlayerId && settings.importKey.length >= 24
      && await extensionApi.permissions.contains({ origins: [permissionOrigin(normalizeBackendUrl(settings.backendUrl))] });
...
function saveConnection() {
  try {
    ...
    const granted = extensionApi.permissions.request({ origins: [permissionOrigin(next.backendUrl)] });
