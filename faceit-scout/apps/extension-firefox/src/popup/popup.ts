...
async function init() {
  ...
  element("save-connection").addEventListener("click", () => {
    try {
      ...
      // Firefox match patterns cannot include ports. The request still uses the configured port.
      const server = new URL(origin);
      extensionApi.permissions.request({ origins: [`${server.protocol}//${server.hostname}/*`] }).then(async granted => {
...
function sendMessage<T>(payload: unknown): Promise<T> {
  return extensionApi.runtime.sendMessage(payload);
}
