...
async function startDownloads(candidates: AnalysisCandidate[], includeProcessed: boolean, requesterNickname?: string, analysisId?: string) {
  ...
  await extensionApi.tabs.create({ url: normalizeBackendUrl(settings.backendUrl) + "/imports?uploads=" + jobs.map(j => j.id).join(","), active: false });
  for (const candidate of pending) await extensionApi.tabs.create({ url: createFaceitMatchroomUrl(candidate.faceitMatchId), active: false });
