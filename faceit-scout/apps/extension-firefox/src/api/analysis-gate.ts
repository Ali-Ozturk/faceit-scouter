...
async function run(backend: string, input: BackendAnalysisInput, key: string) {
  ...
  const saved = (await extensionApi.storage.local.get(bucket))[bucket] as Record | undefined;
  ...
  await extensionApi.storage.local.set({ [bucket]: { key, until: Date.now() + 120000 } });
  try {
    const result = await createAnalysis(backend, input);
    await extensionApi.storage.local.set({ [bucket]: { key, until: Date.now() + 6000, result, expires: Date.now() + 60000 } });
    return result;
  } catch (error) {
    await extensionApi.storage.local.set({ [bucket]: { key, until: Date.now() + 6000 } });
    throw error;
  }
}
