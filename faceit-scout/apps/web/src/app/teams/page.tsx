...
export default async function TeamsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Exact lineups</h1>
        <p className="mt-2 text-slate-600">Lineups are grouped by sorted Steam IDs, not permanent organization names.</p>
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        {teams.map((team) => (
          <Link key={team.id} href={`/teams/${team.id}`} className="rounded border border-slate-200 bg-white p-5 hover:border-slate-400">
            ...
            <p className="mt-2 text-sm text-slate-600">{team.matchCount} matches · {team.maps ?? "No maps yet"}</p>
