...
Allow at least 4 MiB request bodies through the VPS reverse proxy (for nginx, `client_max_body_size 5m;`). Each chunk has a two-minute client timeout. The Node route holds a transaction and shared transfer lock while reading at most one 4 MiB chunk per request, not the entire demo. No FACEIT credentials are needed for uploads or demo parsing; Data API discovery still needs the server-side API token.
...
Upload the original `.dem.zst` or `.dem.gz` downloaded from FACEIT to avoid transferring the larger decompressed `.dem`. Downloads happen in the user's browser; server URL acquisition is disabled. The 4 MiB chunk limit and durable disk sync remain unchanged. Two upload slots stay active as files finish. A global database gate separates upload writes from the complete processing lifecycle (including decompression and persistence). Processing waits until every AWAITING_UPLOAD or UPLOADING reservation across all batches is finished or cancelled; paused or abandoned reservations must be resumed or cancelled. If a new reservation arrives during an existing parse, upload requests retry until that parse finishes. Processing runs one demo at a time while URL/manual imports are enabled, regardless of a higher worker count. The limit of two uploads applies per browser uploader; FACEIT browser downloads are manual. These changes do not claim a measured speedup.
